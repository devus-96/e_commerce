...
export default function WithdrawalForm({
  ...
  const form = useForm<z.infer<typeof withdrawalValidationSchema>>({
    ...
    defaultValues: withdrawalData ? withdrawalData : {},
  });
  ...
  const onSubmit = async (
    ...
    const data = {
      ...
      store: withdrawalData ? withdrawalData.store : store,
      user_id: userId,
    };
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap space-y-4 justify-between items-center">
          ...
          <Link
            href={`/stores/${
              withdrawalData ? withdrawalData.store : store
            }/withdrawals`}
                ...
                <CardContent className="p-10 flex flex-col gap-8">
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      isUpdating ||
                      isLoading ||
                      (withdrawalData && withdrawalData.status === "pending") ||
                      withdrawalData?.status === "pending" ||
                      withdrawalData?.status === "paid"
                    }
                    type="submit"
                  >
                    save
                            ...
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue
                                  defaultValue={field.value}
                                  placeholder={
                                    withdrawalData?.status ?? "Select a status"
                                  }
                                />
                              </SelectTrigger>
                            </FormControl>
